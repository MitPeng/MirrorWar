...
LinkLuaModifier("modifier_item_shine_of_the_madmoon_flutter", "abilities/items/item_shine_of_the_madmoon.lua", LUA_MODIFIER_MOTION_NONE)
--Abilities
...
function modifier_item_shine_of_the_madmoon:GetModifierBonusStats_Strength(params)
	local sotm_table = Load(self:GetParent(), "sotm_table") or {}
	if sotm_table[1] == self then
...
function modifier_item_shine_of_the_madmoon:OnAttackLanded(params)
	...
	if params.attacker == self:GetParent() and not params.attacker:IsIllusion() and not params.attacker:HasModifier("modifier_item_shine_of_the_madmoon_flutter") and not params.attacker:HasModifier("modifier_item_butterfly_custom_flutter") and not params.attacker:HasModifier("modifier_item_jasper_daggers_flutter") and self:GetAbility():IsCooldownReady() then
		local jasper_daggers_table = Load(self:GetParent(), "jasper_daggers_table") or {}
		local sotm_table = Load(self:GetParent(), "sotm_table") or {}
		if #jasper_daggers_table == 0 and sotm_table[1] == self then
			if PRD(self:GetParent(), self.flutter_chance, "flutter_chance") then
				self:GetAbility():UseResources(true, true, true)
				self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_item_shine_of_the_madmoon_flutter", {duration = self.flutter_duration})
			end
		end
	end
end
function modifier_item_shine_of_the_madmoon:GetModifierAttackRangeBonus(params)
...
function modifier_item_shine_of_the_madmoon:GetModifierCastRangeBonusStacking(params)
	if not self:GetParent():IsRangedAttacker() then return end
	local sotm_table = Load(self:GetParent(), "sotm_table") or {}
	if sotm_table[1] == self then
		if IsValid(params.ability) and bit.band(params.ability:GetBehavior(), DOTA_ABILITY_BEHAVIOR_ATTACK) == DOTA_ABILITY_BEHAVIOR_ATTACK then
			return self.base_attack_range
		end
	end
	return 0
...
---------------------------------------------------------------------
--Modifiers
if modifier_item_shine_of_the_madmoon_flutter == nil then
	modifier_item_shine_of_the_madmoon_flutter = class({})
end
function modifier_item_shine_of_the_madmoon_flutter:IsHidden()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:IsDebuff()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:IsPurgable()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:IsPurgeException()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:IsStunDebuff()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:AllowIllusionDuplicate()
	return false
end
function modifier_item_shine_of_the_madmoon_flutter:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_PERMANENT
end
function modifier_item_shine_of_the_madmoon_flutter:OnCreated(params)
	local hParent = self:GetParent()
...
function modifier_item_shine_of_the_madmoon_flutter:OnRefresh(params)
	local hParent = self:GetParent()
...
function modifier_item_shine_of_the_madmoon_flutter:OnDestroy()
	local hParent = self:GetParent()
