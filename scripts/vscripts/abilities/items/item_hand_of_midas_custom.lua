...
function modifier_item_hand_of_midas_custom:OnCreated(params)
	...
	self.bonus_exp = self:GetAbilitySpecialValueFor("bonus_exp")
	...
	if hAttacker ~= nil and hAttacker:GetUnitLabel() ~= "builder" then
		...
		if hAttacker == self:GetParent() and not hAttacker:IsIllusion() then
			...
			if hand_of_midas_table[1] == self and Spawner:IsEndless() == false then
				if PRD(hAttacker, self.chance, "item_hand_of_midas_custom") then
					local bonus_gold = params.unit:IsConsideredHero() and self.bonus_gold * 5 or self.bonus_gold
					local bonus_exp = params.unit:IsConsideredHero() and self.bonus_exp * 5 or self.bonus_exp
					PlayerData:ModifyGold(hAttacker:GetPlayerOwnerID(), bonus_gold, true)
					if hAttacker:IsBuilding() and hAttacker:GetUnitLabel() == "HERO" then
						local hBuilding = hAttacker:GetBuilding()
						hBuilding:AddXP(bonus_exp)
					end
					SendOverheadEventMessage(hAttacker:GetPlayerOwner(), OVERHEAD_ALERT_GOLD, params.unit, bonus_gold, nil)
					local builder = PlayerResource:GetSelectedHeroEntity(hAttacker:GetPlayerOwnerID())
					builder:AddNewModifier(builder, self:GetAbility(), "modifier_item_hand_of_midas_custom_bonus_gold", {bonus_gold=bonus_gold})
				end
			end
		end
	end
