...
LinkLuaModifier("modifier_item_meteor_hammer_custom_check", "abilities/items/item_meteor_hammer_custom.lua", LUA_MODIFIER_MOTION_NONE)
...
function item_meteor_hammer_custom:Meteor(vPosition)
	...
	local overlap_interval = self:GetSpecialValueFor("overlap_interval")
	...
	hCaster:GameTimer(land_time, function()
		...
		for _, hTarget in pairs(tTargets) do
			...
			if not hTarget:HasModifier("modifier_item_meteor_hammer_custom_check") and not hTarget:HasModifier("modifier_item_meteor_grimoire_check") then
				hTarget:AddNewModifier(hCaster, self, "modifier_stunned", {duration=stun_duration*hTarget:GetStatusResistanceFactor()})
				hTarget:AddNewModifier(hCaster, self, "modifier_item_meteor_hammer_custom_check", {duration=overlap_interval})
			end
...
function item_meteor_hammer_custom:IsRefreshable()
	return false
end
function item_meteor_hammer_custom:RefreshCharges()
	local hCaster = self:GetCaster()
...
-------------------------------------------------------------------
if modifier_item_meteor_hammer_custom_check == nil then
	modifier_item_meteor_hammer_custom_check = class({})
end
function modifier_item_meteor_hammer_custom_check:IsHidden()
	return false
end
function modifier_item_meteor_hammer_custom_check:IsDebuff()
	return true
end
function modifier_item_meteor_hammer_custom_check:IsPurgable()
	return false
end
function modifier_item_meteor_hammer_custom_check:IsPurgeException()
	return false
end
function modifier_item_meteor_hammer_custom_check:IsStunDebuff()
	return false
end
function modifier_item_meteor_hammer_custom_check:AllowIllusionDuplicate()
	return false
end
function modifier_item_meteor_hammer_custom_check:OnCreated(params)
	if IsServer() then
	end
end
function modifier_item_meteor_hammer_custom_check:OnDestroy()
	if IsServer() then
	end
end
function modifier_item_meteor_hammer_custom_check:OnIntervalThink()
	if IsServer() then
	end
end