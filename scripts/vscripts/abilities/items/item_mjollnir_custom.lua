...
LinkLuaModifier("modifier_item_mjollnir_custom_cooldown", "abilities/items/item_mjollnir_custom.lua", LUA_MODIFIER_MOTION_NONE)
--Abilities
...
function modifier_item_mjollnir_custom:OnAttackStart(params)
	...
	local mjollnir_table = Load(self:GetParent(), "mjollnir_table") or {}
	if mjollnir_table[1] == self and params.attacker == self:GetParent() and not params.attacker:IsIllusion() and not params.attacker:HasModifier("modifier_item_mjollnir_custom_cooldown") then
		if UnitFilter(params.target, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC+DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, params.attacker:GetTeamNumber()) == UF_SUCCESS then
			if PRD(self:GetParent(), self.chain_chance, "item_mjollnir_custom") then
				...
				params.attacker:AddNewModifier(params.attacker, self:GetAbility(), "modifier_item_mjollnir_custom_cooldown", {duration = self.cooldown})
			end
		end
	end
end
...
---------------------------------------------------------------------
if modifier_item_mjollnir_custom_cooldown == nil then
	modifier_item_mjollnir_custom_cooldown = class({})
end
function modifier_item_mjollnir_custom_cooldown:IsHidden()
	return true
end
function modifier_item_mjollnir_custom_cooldown:IsDebuff()
	return false
end
function modifier_item_mjollnir_custom_cooldown:IsPurgable()
	return false
end
function modifier_item_mjollnir_custom_cooldown:IsPurgeException()
	return false
end
function modifier_item_mjollnir_custom_cooldown:IsStunDebuff()
	return false
end
function modifier_item_mjollnir_custom_cooldown:AllowIllusionDuplicate()
	return false
end
function modifier_item_mjollnir_custom_cooldown:GetPriority()
	return -1
end
