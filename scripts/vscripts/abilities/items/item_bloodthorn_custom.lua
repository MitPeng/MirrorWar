...
function modifier_item_bloodthorn_custom_debuff:OnRefresh(params)
	self.silence_damage_percent = self:GetAbilitySpecialValueFor("silence_damage_percent")
	self.target_crit_multiplier = self:GetAbilitySpecialValueFor("target_crit_multiplier")
	if IsServer() then
		self:StartIntervalThink(self:GetDuration())
	...
	if IsServer() then
		if self.damage > 0 then
			...
			local damage_table = 
			{
				...
				damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION+DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS+DOTA_DAMAGE_FLAG_NO_DIRECTOR_EVENT+DOTA_DAMAGE_FLAG_PROPERTY_FIRE,
			}
...
function modifier_item_bloodthorn_custom_debuff:OnTakeDamage(params)
	if params.unit == self:GetParent() then
		if bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_PROPERTY_FIRE) ~= DOTA_DAMAGE_FLAG_PROPERTY_FIRE then
