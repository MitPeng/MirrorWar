...
function modifier_item_crescent_bow:GetModifierBonusStats_Strength(params)
...
function modifier_item_crescent_bow:GetModifierAttackRangeBonus(params)
	if not self:GetParent():IsRangedAttacker() then return end
	local crescent_bow_table = Load(self:GetParent(), "crescent_bow_table") or {}
	local sotm_table = Load(self:GetParent(), "sotm_table") or {}
	if #sotm_table == 0 and crescent_bow_table[1] == self then
		return self.base_attack_range
	end
end
function modifier_item_crescent_bow:GetModifierCastRangeBonusStacking(params)
	if not self:GetParent():IsRangedAttacker() then return end
	local crescent_bow_table = Load(self:GetParent(), "crescent_bow_table") or {}
	local sotm_table = Load(self:GetParent(), "sotm_table") or {}
	if #sotm_table == 0 and crescent_bow_table[1] == self then
...
function modifier_item_crescent_bow:GetModifierAttackSpeedBonus_Constant(params)
