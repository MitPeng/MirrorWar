...
LinkLuaModifier("modifier_item_meteor_grimoire_check", "abilities/items/item_meteor_grimoire.lua", LUA_MODIFIER_MOTION_NONE)
	...
	hCaster:GameTimer(land_time, function()
		...
		for _, hTarget in pairs(tTargets) do
			...
			if not hTarget:HasModifier("modifier_item_meteor_hammer_custom_check") and not hTarget:HasModifier("modifier_item_meteor_grimoire_check") then
				...
				hTarget:AddNewModifier(hCaster, self, "modifier_item_meteor_grimoire_check", {duration=overlap_interval})
			end
...
function item_meteor_grimoire:IsRefreshable()
	return false
end
function item_meteor_grimoire:RefreshCharges()
	local hCaster = self:GetCaster()
...
function modifier_item_meteor_grimoire:OnDestroy()
	...
	if self.key ~= nil then
		SetSpellCriticalStrike(hParent, nil, nil, self.key)
	end
...
-------------------------------------------------------------------
if modifier_item_meteor_grimoire_check == nil then
	modifier_item_meteor_grimoire_check = class({})
end
function modifier_item_meteor_grimoire_check:IsHidden()
	return false
end
function modifier_item_meteor_grimoire_check:IsDebuff()
	return true
end
function modifier_item_meteor_grimoire_check:IsPurgable()
	return false
end
function modifier_item_meteor_grimoire_check:IsPurgeException()
	return false
end
function modifier_item_meteor_grimoire_check:IsStunDebuff()
	return false
end
function modifier_item_meteor_grimoire_check:AllowIllusionDuplicate()
	return false
end
function modifier_item_meteor_grimoire_check:OnCreated(params)
	if IsServer() then
	end
end
function modifier_item_meteor_grimoire_check:OnDestroy()
	if IsServer() then
	end
end
function modifier_item_meteor_grimoire_check:OnIntervalThink()
	if IsServer() then
	end
end